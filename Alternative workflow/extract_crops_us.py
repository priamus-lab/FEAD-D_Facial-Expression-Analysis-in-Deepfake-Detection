    ...
    try:
        ...
        for i in range(frames_num):
            ...
            if bboxes is None:
                ...
                if last_saved >=0:
                  #backward
                  bboxes = bboxes_dict[str(last_saved)]
                  counter += 1
                  go = True
                  
                else:
                  #forward
                  idx = i #dovrebbe essere il primo
                  trovato = False

                  while idx < frames_num and trovato==False:
                    if bboxes_dict[str(idx)] is None:
                      idx = idx + 1
                    else:
                      trovato = True
            ...
            if go == True:
              ...
              for j, crop in enumerate(crops):
                os.makedirs(os.path.join(opt.output_path, id, str(j)), exist_ok=True)
                cv2.imwrite(os.path.join(opt.output_path, id, str(j), "{}_{}.png".format(i, j)), crop)
